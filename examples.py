"""
Usage examples for GoEmotions multi-label emotion classification.
Demonstrates different functionalities: downsampling, label analysis, and augmentation.
"""
...
def example_downsampling():
    ...
    print("Running downsampling experiments...")
    pipeline = GoEmotionsTrainingPipeline()
    
    # Run downsampling experiments with fewer levels for faster execution
    results = pipeline.run_downsampling_experiments(
        reduction_percentages=[30, 60, 90],
        random_state=42
    )
...
def example_label_analysis():
    """Example of label analysis using regression to identify underperforming labels."""
    print("Running label analysis with regression...")
    pipeline = GoEmotionsTrainingPipeline()
...
def example_single_downsample():
    ...
    print("Single downsampling example...")
...
def example_augmentation_experiment():
    ...
    print("Running augmentation experiment (EDA + LLM)...")
    pipeline = GoEmotionsTrainingPipeline()
    ...
    if underperforming_labels:
        ...
        results = pipeline.run_augmentation_experiments(
            underperforming_labels=underperforming_labels[:2],  # Test first 2 labels only
            reduction_pct=60.0,
            target_count=3166,
